...
function ProviderPlan({ plan, i18n, pending = false }) {
  ...
  const price = Number.isFinite(amount)
    ? i18n.formatMoney(amount, plan.price.currency || "GBP", { maximumFractionDigits: 2 })
    : plan.price.amount;
...
export default function SubscriptionChangePanel({ merchantUi, current, pending, providerVerificationState, managePlansHref, managePlansAvailable }) {
  ...
  return <section className="moda-billing-panel">
    ...
    {hasCurrent ? <>
      <ProviderPlan i18n={i18n} plan={{ shopifyPlanHandle: current.shopifyPlanHandle, price: current.price, interval: current.interval }} />
      <p>{current.mappedModaPlanName || i18n.t("billing.unknownPlan")}{current.cancelAtEndOfCycle ? ` - ${i18n.t("billing.cancelAtPeriodEnd")}` : ""}</p>
...
SubscriptionChangePanel.propTypes = {
  ...
  pending: PropTypes.shape({ shopifyPlanHandle: PropTypes.string.isRequired, mappedModaPlanName: PropTypes.string, price: PropTypes.shape({ amount: PropTypes.string.isRequired, currency: PropTypes.string }).isRequired, interval: PropTypes.string.isRequired, effectiveAt: PropTypes.string }),
