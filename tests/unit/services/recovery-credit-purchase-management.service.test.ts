...
function purchase(
  ...
  return {
    ...
    plan: { name: "Growth", shopifyPlanHandle: "growth" },
    providerPurchaseAmount: "12.50",
  ...
  const transaction: any = {
    ...
    recoveryCreditRefund: {
      create: vi.fn(async ({ data }: any) => {
        ...
        refunds.set(data.requestKey, refund);
        return data;
      }),
    ...
    shopEntitlementCounter: {
      ...
      updateMany: vi.fn(async ({ where, data }: any) => {
        if (where.version !== aggregate.version) return { count: 0 };
        aggregate.refundingQuantity +=
          data.refundingQuantity.increment ?? -data.refundingQuantity.decrement;
        aggregate.version += 1;
        return { count: 1 };
      }),
    },
...
describe("RecoveryCreditPurchaseManagementService", () => {
  it("lists only the authenticated shop with bounded pagination and computed availability", async () => {
    ...
    const result = await new RecoveryCreditPurchaseManagementService(
      fixture.database,
    ).listPurchaseHistory({ shopId: "shop-1", pageSize: 100 });
  ...
  it("filters status before pagination with one shared shop predicate", async () => {
    ...
    const active = await service.listPurchaseHistory({
      shopId: "shop-1",
      page: 1,
      pageSize: 2,
    ...
    expect(
      active.purchases.every(
        (row) => row.status === RecoveryCreditPurchaseStatus.ACTIVE,
      ),
    ).toBe(true);
  ...
  it("maps every canonical purchase status without exposing provider internals", async () => {
    ...
    expect(result.purchases.map(({ status }) => status)).toEqual([
      "REQUESTED",
      "ACTIVE",
      "COMPLETED",
      "WITHDRAWN",
  ...
  it("withdraws fresh available capacity and holds it in the aggregate", async () => {
    ...
    expect(
      fixture.transaction.recoveryCreditRefund.create,
    ).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("requires the current provider context before creating a refund hold", async () => {
    ...
    expect(provider.getActiveSubscription).toHaveBeenCalledWith({
      shopifyShopId: "gid://shopify/Shop/1",
    });
  });

  it.each([
    ["prior plan", { planHandle: "starter" }],
    [
      "prior billing cycle",
      {
    ...
    ["missing event handle", { usageEventHandles: [] }],
  ])("keeps a %s purchase active", async (_label, overrides) => {
    const current = providerSubscription();
    const fixture = database([purchase("one")]);
    fixture.rows.get("one").providerSubscriptionIdSnapshot =
      deriveShopifyProviderContextIdentity(current);
    const result = await new RecoveryCreditPurchaseManagementService(
      fixture.database,
      {
        getActiveSubscription: vi
          .fn()
          .mockResolvedValue(providerSubscription(overrides)),
      } as any,
    ).requestRefund({
      shopId: "shop-1",
      shopifyShopId: "gid://shopify/Shop/1",
  ...
  it("accepts a current native App Pricing context without a legacy provider ID", async () => {
    const current = providerSubscription({ providerSubscriptionId: null });
    const fixture = database([purchase("one")]);
    fixture.rows.get("one").providerSubscriptionIdSnapshot =
      deriveShopifyProviderContextIdentity(current);
    const result = await new RecoveryCreditPurchaseManagementService(
  ...
  it("replays a live refund without creating another row", async () => {
    ...
    ).resolves.toMatchObject({ code: "REQUESTED" });
    expect(
      fixture.transaction.recoveryCreditRefund.create,
  ...
  it("retries the whole refund transaction after a serialization conflict", async () => {
    ...
    fixture.database.$transaction = vi
      ...
      .mockRejectedValueOnce(
        ...
        ).Prisma.PrismaClientKnownRequestError("serialization failure", {
          code: "P2034",
          clientVersion: "6",
        }),
      )
  ...
  it("reactivates a pre-provider refund and releases only its held available amount", async () => {
    ...
    const result = await new RecoveryCreditPurchaseManagementService(
      fixture.database,
    ).reactivateRefund({ shopId: "shop-1", purchaseId: "one" });
  ...
  it("rejects provider-action refunds and completes a withdrawn empty purchase", async () => {
    ...
    await expect(
      new RecoveryCreditPurchaseManagementService(
        locked.database,
      ).reactivateRefund({ shopId: "shop-1", purchaseId: "locked" }),
    ).resolves.toEqual({
      purchaseId: "locked",
    ...
    await expect(
      new RecoveryCreditPurchaseManagementService(
        empty.database,
      ).reactivateRefund({ shopId: "shop-1", purchaseId: "empty" }),
    ).resolves.toEqual({ purchaseId: "empty", code: "COMPLETED_NO_CREDITS" });
  });
});
