...
const inputFor = (state: string) => {
  if (state === "REINSTALLING")
    return { shop: { status: "UNINSTALLED", reinstallPendingAt: new Date() } };
...
describe("merchant route access policy", () => {
  ...
  it.each(MERCHANT_EXPERIENCE_STATES)(
    "has an explicit decision for every surface in %s",
    (state) => {
      expect(
        MERCHANT_SURFACES.every(
          (surface) =>
            typeof canAccessMerchantSurface(state, surface) === "boolean",
        ),
      ).toBe(true);
    },
  );

  it("matches the exact lifecycle matrix without restoring onboarding billing", () => {
    const expected = {
      SIGNED_OUT: [],
    ...
    for (const state of MERCHANT_EXPERIENCE_STATES) {
      expect(
        MERCHANT_SURFACES.filter((surface) =>
  ...
  it.each([
    ...
    "returns the denial destination for %s/%s",
    (state, surface, destination) => {
  ...
  it.each([
    ...
    (state, hrefs) => {
      expect(getMerchantNavigation(state).map((item) => item.href)).toEqual(
        hrefs,
      );
      expect(
